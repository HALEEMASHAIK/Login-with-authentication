...
function SSOButtons({ onSSO, buttonText = "Continue with" }) {
  const handleSSOLogin = async (provider) => {
    try {
      ...
      const authUrl = authAPI.getSSOAuthorizationUrl(provider, redirectUri);
      ...
      const ssoData = mockSSOData[provider];
      if (ssoData) {
        // Call the parent SSO handler with the email
        onSSO(ssoData.email);
        
        // Store additional SSO data for potential use
